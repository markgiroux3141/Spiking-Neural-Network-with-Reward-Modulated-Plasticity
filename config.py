...
class Config:
    ...
    baseline_tau: float = 5_000.0   # ms EMA window for expected reward (~5 s)
    ...
    # potential-based reward shaping (SPEC §10 credit-assignment aid): dense
    # signal for approaching reward / leaving punishment, eases RL-STDP learning.
    shaping_gain: float = 8.0    # 0 disables (back to sparse region-only reward)
    ...
    def derived(self) -> dict:
        ...
        n_input = self.n_rays * len(self.vision_channels)
        return {
            "n_input": n_input,
